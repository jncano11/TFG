...
// Solo Admin o Entrenador de este equipo pueden cambiar escudo
if (
...
// Procesar subida
if (empty($_FILES['escudo']) || $_FILES['escudo']['error'] !== UPLOAD_ERR_OK) {
    die('Error al subir el archivo.');
}

$archivo = $_FILES['escudo'];
// Validar tipo MIME (solo imágenes jpg/png/gif)
...
$nombreFoto   = 'escudo_' . $equipoId . '_' . uniqid() . $ext;
...
if ($equipoModel->actualizarEscudo($equipoId, $rutaBD)) {
    // Redirigir de vuelta al perfil con flag de éxito
    header("Location: ../views/equipo_perfil.php?id={$equipoId}&escudo=ok");
    exit();
} else {
